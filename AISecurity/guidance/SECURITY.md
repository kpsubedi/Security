...
| `{{BACKEND_FRAMEWORK_PROMPT}}` | RESOLVED | `REF-PROMPT-NODE` — `/<>/PROMPT.md`, read and applied. The specific Node.js server framework is TO BE DECIDED, so framework-specific sub-prompts were not selected. |
| `{{FRONTEND_FRAMEWORK_PROMPT}}` | RESOLVED | `REF-PROMPT-VUE` — `/<>/PROMPT.md`, read and applied. |
